<?php if($_SESSION['id'] == 1){require($dirAssets.'check_schede_alert.php');} ?>
<div class="row">
  <div class="col-md-8"><?php require($dirAssets.'card_filtra_schede.php'); ?></div>
  <div class="col-md-4"><?php require($dirAssets.'card_schedatori.php'); ?></div>
</div>
<div class="row mt-3">
  <div class="col-12"><?php require($dirAssets.'card_mappa.php'); ?></div>
</div>
<div class="row mt-3">
  <div class="col-md-8"><?php require($dirAssets.'card_utenti.php'); ?></div>
  <div class="col-md-4"><?php require($dirAssets.'card_comunicazioni.php'); ?></div>
</div>
<div class="row mt-3">
  <div class="col-md-8"><?php require($dirAssets.'card_biblio.php'); ?></div>
  <div class="col-md-4"><?php require($dirAssets.'card_stato_schede.php'); ?></div>
</div>
